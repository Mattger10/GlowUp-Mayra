...
interface BlusasDetailsProps {
  addToCart: (product: BlusasyCamisas) => void;
}

const BlusasDetails: FunctionComponent<BlusasDetailsProps> = ({ addToCart }) => {
  const { id } = useParams<{ id: string }>();
  const { lista } = useFirestoreBlusasyCamisas();
  ...
  const [cartItems, setCartItems] = useState<{ [key: number]: BlusasyCamisas }>({});
  ...
  const handleAddToCart = async (product: BlusasyCamisas) => {
