...
import useFirestoreCorset, {
  Corset,
} from "../Components/Firestore/FirestoreHandlerCorset";
import Loading from "./Loading";

interface CorsetDetailsProps {
  addToCart: (product: Corset) => void;
}

const CorsetDetails: FunctionComponent<CorsetDetailsProps> = ({ addToCart }) => {
  const { id } = useParams<{ id: string }>();
  const { lista } = useFirestoreCorset();
  ...
  const [cartItems, setCartItems] = useState<{ [key: number]: Corset }>({});
  ...
  const handleAddToCart = async (product: Corset) => {
