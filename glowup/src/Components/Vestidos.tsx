...
interface VestidosyEnterizosProps {
  addToCart: (product: VestidosyEnterizos) => void;
}

const VestidosyEnterizosPage: FunctionComponent<VestidosyEnterizosProps> = ({ addToCart }) => {
  // const { Accesorios } = products;

  const [isCartModalOpen, setIsCartModalOpen] = useState<boolean>(false);
  const [selectedProduct, setSelectedProduct] = useState<VestidosyEnterizos | null>(
    null
  );
  ...
  const handleOpenCartModal = (product: VestidosyEnterizos) => {
        ...
        <GridContainer>
          {lista.map((producto, index) => (
            <ProductContainer key={producto.id.toString()}>
              <CustomLink
                to={`/vestidosyenterizosdetails/${producto.id}`}
...
export default VestidosyEnterizosPage;
