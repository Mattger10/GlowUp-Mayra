...
const Details: FunctionComponent<DetailsProps> = ({ addToCart }) => {
  const { id } = useParams<{ id: string }>();
  const { lista } = useFirestoreProductos();
  const product = lista.find((producto) => producto.id === id);

  const [selectedSize, setSelectedSize] = useState<string>(
    product && product.talles.length > 0 ? product.talles[0] : ""
  );
  ...
  const [cartItems, setCartItems] = useState<{ [key: number]: Producto }>({});
