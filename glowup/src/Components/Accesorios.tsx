...
const AccesoriosPage: FunctionComponent<AccesoriosProps> = ({ addToCart }) => {
  // const { Accesorios } = products;

  const [isCartModalOpen, setIsCartModalOpen] = useState<boolean>(false);
  const [selectedProduct, setSelectedProduct] = useState<Accesorios | null>(
    null
  );
  ...
  const handleOpenCartModal = (product: Accesorios) => {
        ...
        <GridContainer>
          {lista.map((producto, index) => (
            <ProductContainer key={producto.id.toString()}>
              <CustomLink
                to={`/accesoriosdetails/${producto.id}`}
