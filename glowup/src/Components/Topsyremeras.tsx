...
import useFirestoreTopsyRemeras, {
 TopsyRemeras,
} from "../Components/Firestore/FirestoreHandlerTopsyRemeras";
import Loading from "./Loading";

interface TopsyRemerasProps {
  addToCart: (product: TopsyRemeras) => void;
}

const TopsyRemerasPage: FunctionComponent<TopsyRemerasProps> = ({ addToCart }) => {
  // const { Accesorios } = products;

  const [isCartModalOpen, setIsCartModalOpen] = useState<boolean>(false);
  const [selectedProduct, setSelectedProduct] = useState<TopsyRemeras | null>(
    null
  );
  const [selectedSize, setSelectedSize] = useState<string>("");
  const [color, setColor] = useState("");
  const { lista } = useFirestoreTopsyRemeras();
  const [hoveredIndex, setHoveredIndex] = useState<number | null>(null);

  const handleOpenCartModal = (product: TopsyRemeras) => {
        ...
        <GridContainer>
          {lista.map((producto, index) => (
            <ProductContainer key={producto.id.toString()}>
              <CustomLink
                to={`/topsyremerasdetails/${producto.id}`}
...
export default TopsyRemerasPage;
