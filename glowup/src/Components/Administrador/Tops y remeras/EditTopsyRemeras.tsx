...
import useFirestoreTopsyRemeras, { TopsyRemeras } from "../../Firestore/FirestoreHandlerTopsyRemeras";
import {
  Button,
  TextField,
  Typography,
...
interface EditTopsyRemerasProps {}

const EditTopsyRemeras: FunctionComponent<EditTopsyRemerasProps> = () => {
  const { id } = useParams<{ id: string }>();
  const { lista, actualizarTopsyRemeras } = useFirestoreTopsyRemeras();
  const [formData, setFormData] = useState<Partial<TopsyRemeras>>({
  ...
  useEffect(() => {
    const topsyremeras = lista.find((producto) => producto.id === id);
    if (topsyremeras) {
      setFormData((prevData) => ({
        ...prevData,
        ...topsyremeras,
        colores: Array.isArray(topsyremeras.colores) ? topsyremeras.colores : [],
        talles: Array.isArray(topsyremeras.talles) ? topsyremeras.talles : [],
      }));
    }
  }, [id, lista]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    await actualizarTopsyRemeras(id, formData);
    ...
    setLoading(false);
    navigate("/administrador/topsyremeras"); // Redirigir al usuario a la ruta deseada
  };
...
export default EditTopsyRemeras;
