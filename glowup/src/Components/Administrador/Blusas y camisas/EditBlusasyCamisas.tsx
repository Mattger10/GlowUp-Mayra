...
import { BlusasyCamisas } from "../../Firestore/FirestoreHandlerBlusasyCamisas";
import {
  Button,
  TextField,
  Typography,
...
import useFirestoreBlusasyCamisas from "../../Firestore/FirestoreHandlerBlusasyCamisas";
...
interface EditBlusasyCamisasProps {}

const EditBlusasyCamisas: FunctionComponent<EditBlusasyCamisasProps> = () => {
  const { id } = useParams<{ id: string }>();
  const { lista, actualizarBlusasyCamisas } = useFirestoreBlusasyCamisas();
  const [formData, setFormData] = useState<Partial<BlusasyCamisas>>({
  ...
  useEffect(() => {
    const accesorio = lista.find((producto) => producto.id === id);
    if (accesorio) {
      setFormData((prevData) => ({
        ...prevData,
        ...accesorio,
        colores: Array.isArray(accesorio.colores) ? accesorio.colores : [],
        talles: Array.isArray(accesorio.talles) ? accesorio.talles : [],
      }));
    }
  }, [id, lista]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    await actualizarBlusasyCamisas(id, formData);
    ...
    setLoading(false);
    navigate("/administrador/blusasycamisas"); // Redirigir al usuario a la ruta deseada
  };
...
export default EditBlusasyCamisas;
