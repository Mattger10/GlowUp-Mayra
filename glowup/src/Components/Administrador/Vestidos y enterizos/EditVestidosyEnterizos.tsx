...
import useFirestoreVestidosyEnterizos, { VestidosyEnterizos } from "../../Firestore/FirestoreHandlerVestidosyEnterizos";
import {
  Button,
  TextField,
  Typography,
...
interface EditVestidosyEnterizosProps {}

const EditVestidosyEnterizos: FunctionComponent<EditVestidosyEnterizosProps> = () => {
  const { id } = useParams<{ id: string }>();
  const { lista, actualizarVestidosyEnterizos } = useFirestoreVestidosyEnterizos();
  const [formData, setFormData] = useState<Partial<VestidosyEnterizos>>({
  ...
  useEffect(() => {
    const vestidosyenterizos = lista.find((producto) => producto.id === id);
    if (vestidosyenterizos) {
      setFormData((prevData) => ({
        ...prevData,
        ...vestidosyenterizos,
        colores: Array.isArray(vestidosyenterizos.colores) ? vestidosyenterizos.colores : [],
        talles: Array.isArray(vestidosyenterizos.talles) ? vestidosyenterizos.talles : [],
      }));
    }
  }, [id, lista]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Actualizar "Tops y remeras" siempre
    await actualizarVestidosyEnterizos(id, formData);
    ...
    setLoading(false);
    navigate("/administrador/vestidosyenterizos"); // Redirigir al usuario a la ruta deseada
  };
...
export default EditVestidosyEnterizos;
