...
import { Pantalones } from "../../Firestore/FirestoreHandlerPantalones";
import {
  Button,
  TextField,
  Typography,
...
import useFirestorePantalones from "../../Firestore/FirestoreHandlerPantalones";
...
interface EditPantalonesProps {}

const EditPantalones: FunctionComponent<EditPantalonesProps> = () => {
  const { id } = useParams<{ id: string }>();
  const { lista, actualizarPantalones } = useFirestorePantalones();
  const [formData, setFormData] = useState<Partial<Pantalones>>({
  ...
  useEffect(() => {
    const pantalones = lista.find((producto) => producto.id === id);
    if (pantalones) {
      setFormData((prevData) => ({
        ...prevData,
        ...pantalones,
        colores: Array.isArray(pantalones.colores) ? pantalones.colores : [],
        talles: Array.isArray(pantalones.talles) ? pantalones.talles : [],
      }));
    }
  }, [id, lista]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    await actualizarPantalones(id, formData);
    ...
    setLoading(false);
    navigate("/administrador/pantalones"); // Redirigir al usuario a la ruta deseada
  };
