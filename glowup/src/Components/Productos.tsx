...
import useFirestoreProductos, { Producto } from "./Firestore/FirestoreHandler";

interface ProductosProps {
  addToCart: (product: Producto) => void; // Asegúrate de que addToCart acepte cualquier tipo de producto
}

const Productos: React.FC<ProductosProps> = ({ addToCart }) => {
  const { lista } = useFirestoreProductos();
  ...
  const handleOpenCartModal = (product: Producto) => {
    setSelectedProduct(product);
    setSelectedSize(product.talles[0]); // Establece el primer tamaño por defecto
    setColor(product.colores[0]); // Establece el primer color por defecto
        ...
        <GridContainer>
          {lista.map((producto, index) => (
            <ProductContainer key={producto.id.toString()}>
              <CustomLink to={`/details/${producto.id}`}>
                <ImageContainer
...
const HoverImg = styled(Img)<{ visible: boolean }>`
  position: absolute;
  top: 0;
  left: 0;
